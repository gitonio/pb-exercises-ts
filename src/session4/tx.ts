...
export class TxFetcher {
	...
	fetch(txId: number, testnet=false, fresh=false) {
		if (fresh || !(txId in this.cache)) {
			const url = this.getUrl(testnet) + '/tx/' + txId.toString(16);
...
export class TxIn {
  constructor(
    ...
    public sequence: number = parseInt('0xffffffff',16),
    public cache: object = {}
  ) {}
  ...
  static parse(s: Readable): TxIn {
	...
	serialize(): Buffer {
		...
		let fresult = Buffer.concat([result , helper.intToLittleEndian(this.prevIndex, 4)]);
		...
		fresult = Buffer.concat([fresult, helper.intToLittleEndian(this.sequence,4)]);
		return fresult;
	}
/*	
