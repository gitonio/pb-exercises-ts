var { assert, expect } = require('chai');
import { Tx } from './tx';
import { Script, p2pkhScript, p2shScript } from './script';
var Readable = require('stream').Readable;
var ecc = require('./ecc');
var helper = require('./helper');
import { decodeBase58 } from './helper';

describe('ScriptTest', function() {
  const rawTx = Buffer.from(
    '',
    'hex'
  );
  const scriptPubkey = Buffer.from(
    '6a47304402207899531a52d59a6de200179928ca900254a36b8dff8bb75f5f5d71b1cdc26125022008b422690b8461cb52c3cc30330b23d574351872b7c361e9aae3649071c1a7160121035d5c93d9ac96881f19ba1f686f15f009ded7c62efe85a872e6a19b43c15a2937',
    'hex'
  );
  const readable = new Readable();
  readable.push(scriptPubkey);
  readable.push(null);

  // const tx = Tx.parse(readable);
  // console.log(tx);
  // console.log(tx.inputs[1].scriptSig);
  // console.log(tx.outputs[0].scriptPubkey);
  const script = Script.parse(readable);
  //const script = Script.parse(tx.inputs[1].scriptSig.elements[1] as Buffer);
  const want = Buffer.from(
    '304402207899531a52d59a6de200179928ca900254a36b8dff8bb75f5f5d71b1cdc26125022008b422690b8461cb52c3cc30330b23d574351872b7c361e9aae3649071c1a71601',
    'hex'
  );

  //const readable = new Readable();
  //readable.push(rawTx);
  //readable.push(null);

  //  const tx = Tx.parse(readable);
  //  console.log('here:', tx.outputs);
  //  const sc = Script.parse(rawTx2);
  it('test_parse', function() {
    assert.equal(
      (script.elements[0] as Buffer).toString('hex'),
      want.toString('hex')
    );
    assert.equal(
      (script.elements[1] as Buffer).toString('hex'),
      Buffer.from(
        '035d5c93d9ac96881f19ba1f686f15f009ded7c62efe85a872e6a19b43c15a2937',
        'hex'
      ).toString('hex')
    );
  });

  it('test_serialize', function() {
    assert.deepEqual(script.serialize(), scriptPubkey);
  });

  it('test_address', function() {
    const address1 = '1BenRpVUFK65JFWcQSuHnJKzc4M8ZP8Eqa';
    let h160 = decodeBase58(address1);
    const p2pkhScriptPubkey = p2pkhScript(h160);
    assert.equal(p2pkhScriptPubkey.address(), address1);

    const address2 = 'mrAjisaT4LXL5MzE81sfcDYKU3wqWSvf9q';
    assert.equal(p2pkhScriptPubkey.address(true), address2);

    const address3 = '3CLoMMyuoDQTPRD3XYZtCvgvkadrAdvdXh';
    h160 = decodeBase58(address3);
    const p2shScriptPubkey = p2shScript(h160);
    assert.equal(p2shScriptPubkey.address(), address3);

    const address4 = '2N3u1R6uwQfuobCqbCgBkpsgBxvr1tZpe7B';
    assert.equal(p2shScriptPubkey.address(true), address4);
  });
});
